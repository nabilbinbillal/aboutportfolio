...
function ee(e) {
    let n, c, r, isActive = false; // Track the active state of the button

    return {
        c() {
            n = document.createElement("button"); // Create a button element
            n.textContent = "SPACE MODE"; // Set button text
            n.classList.add("svelte-6brq6f", "visible-button"); // Add classes for styling
            n.addEventListener("click", () => {
                isActive = !isActive; // Toggle active state
                n.classList.toggle("hovered", isActive); // Add or remove hovered class based on active state
            });
        },
        m(t, e) {
            t.insertBefore(n, e); // Insert button into DOM
            c || (r = n.addEventListener("click", () => { /* Click handler already attached in c() */ }), c = true);
        },
        p: t,
        i: t,
        o: t,
        d(t) {
            if (t) n.remove(); // Remove button from DOM
            c = false; // Reset click event
            r(); // Clean up event listeners
        }
    };
}
